...
def set_prompt_root(path: Union[str, Path]) -> None:
    """设置 prompt 模板根目录"""
    global PROMPT_ROOT
    PROMPT_ROOT = Path(path).resolve()


def _get_prompt_root() -> Path:
    """返回当前 prompt 模板根目录，未配置时使用包同级 resources/prompts"""
    if PROMPT_ROOT is not None:
        return PROMPT_ROOT
    return Path(__file__).resolve().parent.parent / "resources" / "prompts"


def get_prompt_template_path(name: str) -> Path:
    """根据模板名称返回 Markdown 模板路径"""
    root = _get_prompt_root()
    candidate = root / f"{name}.md"
    if not candidate.exists():
        raise FileNotFoundError(
            f"未找到名为 '{name}' 的prompt模板: {candidate}\n"
            f"提示: 可通过 set_prompt_root() 指定模板目录"
        )
    return candidate
